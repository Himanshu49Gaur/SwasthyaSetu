...
The frontend is fully configured with **Capacitor** to be wrapped into a native Android app.

1. Ensure your frontend is built:
   ```bash
   cd frontend
   npm run build
   ```
2. Sync the compiled assets to the Android platform:
   ```bash
...
3. Open the project in Android Studio or build it via the command line:
   ...
   ```
Your compiled `.apk` file will be located at `frontend/android/app/build/outputs/apk/debug/app-debug.apk`.

> **Note:** The Android app will look for the backend URL defined in `frontend/.env`. Be sure to set `VITE_API_URL` to your production backend URL before building the APK for actual mobile distribution.

---

## 🐳 Docker Deployment (Backend)

Because Vercel Serverless Functions have a 250MB size limit (and PyTorch is over 800MB), the backend should be hosted using Docker on platforms like Render, Railway, or Hugging Face Spaces.

A `Dockerfile` is included at the root of the project.
```bash
# Build the Docker image
docker build -t swasthyasetu-backend .

# Run the Docker container
docker run -p 8000:8000 swasthyasetu-backend
```
